...
const bcgovBlockThemePlugin = {
	initFrontendDriverCategoryQuery: function () {
		...
		*/
		requestAnimationFrame(() => {
			...
			const queryLoopDriverCardContainer = document.querySelectorAll('.wp-block-query.vue-card-container');
			 ...
			 * @param {number} [offset=0] - The offset for paginating through API results.
			 ...
			 * @returns {Promise<void>} A Promise that resolves after fetching, processing, and replacing links.
			 */
			async function fetchDataAndReplaceLinks(page = 1) {
				try {
					const perPage = 100;
					const cachedData = localStorage.getItem('apiData');
					const cachedTimestamp = localStorage.getItem('apiDataTimestamp');
					...
					if (cachedData && cachedTimestamp) {
						try {
							...
							console.error('Error parsing cached data:', cacheError.message);
						}
					}

					const response = await fetch(`https://cleanbc.gov.bc.ca/wp-json/wp/v2/project?_embed&per_page=${perPage}&page=${page}&_category_image=true`);
			 ...
			 * @returns {void} The function does not return a value, but it logs the unique categories and calls replaceLinks with the processed data.
			 */
			 ...
			 * @returns {void} The function does not return a value, but it modifies the DOM by replacing links with images.
			 */
			function replaceLinks(categories) {
				document.querySelectorAll('div.taxonomy-category').forEach(categoryDiv => {
					...
					const separators = categoryDiv.querySelectorAll('span.wp-block-post-terms__separator');
					...
					links.forEach(link => {
						...
						if (matchingCategory) {
							const categoryImage = matchingCategory?.acf?.category_image;
							if (categoryImage) {
...
if ('complete' === document.readyState) {
	bcgovBlockThemePlugin.initFrontendDriverCategoryQuery();
} else {
	document.addEventListener('DOMContentLoaded', bcgovBlockThemePlugin.initFrontendDriverCategoryQuery());
}