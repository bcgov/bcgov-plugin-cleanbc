...
const fieldErrors = computed(() => {
  ...
  const pagePathWaterHeatingTokens =
    typeof window !== 'undefined' ? extractHeatingTokens(window.location.pathname) : []
...
function setSingleModeDialogScrollLock(isLocked) {
  ...
  if (isLocked) {
    ...
    body.dataset.singleModeDialogScrollY = String(scrollY)
    html.style.margin = '0'
  ...
  const savedScrollY = Number.parseInt(body.dataset.singleModeDialogScrollY || '0', 10) || 0
  delete body.dataset.singleModeDialogScrollY
  html.style.margin = ''
  html.style.height = ''
  html.style.overflow = ''
  body.style.margin = ''
