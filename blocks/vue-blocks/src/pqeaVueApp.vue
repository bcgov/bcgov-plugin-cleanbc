<template>
    ...
    <!-- Filter Controls -->
    <div v-if="!isLoading" class="pqeasFilterControls" id="pqeasFilterControls">
        <!-- Category Select -->
        <label for="categorySelect" class="sr-only">Choose between home construction and home renovation</label>
        <div class="custom-select">
            <select @click.prevent="selectIsActive" @touchend="selectIsActive" @keyup.esc="selectIsActive" tabindex="0" id="categorySelect" class="select select--category" v-model="selectedCategory" :required="true" data-active="false">
                <option v-for="(category, index) in categories" :key="category" :value="category">{{ category }}</option>
            </select>
        </div>
        ...
        <div class="custom-select">
            <select @click.prevent="selectIsActive" @touchend="selectIsActive" @keyup.esc="selectIsActive" tabindex="0" id="locationSelect" class="select select--location" v-model="selectedLocation">
        ...
        <!-- Pagination Controls -->
        <div v-if="!isLoading" class='pqeasFilterPagination pqeas-filter__pagination pqeas-filter__pagination--top'>
            <template v-if='filteredPqeas.length > 0'>
                <button class="prevPage" @click.prevent="prevPage" :disabled="currentPage === 1" tabindex='0'>Previous Page</button>
                <span class='pages'>Page <span class='numValue'>{{ currentPage }}</span> of <span class='numValue'>{{ totalPages }}</span></span>
                <button class="nextPage" @click.prevent="nextPage" :disabled="currentPage === totalPages" tabindex='0'>Next Page</button>
            </template>

            <!-- Results Information -->
            <div class='totals' aria-live="polite" aria-atomic='true' tabindex='0'>
                Showing <span class="results-count"><span class='numValue'>{{paginatedPqeas.length }}</span> of <span class='numValue'>{{ filteredPqeas.length }}</span></span> Energy Advisors
                <span class="sr-only">{{ currentTypeFilterMessage }} {{ currentLocationFilterMessage }}</span>
            </div>
        </div>
    </div>

    <!-- Loading Message -->
    <p v-if="isLoading" class="no-results loading" aria-live='polite'>Retrieving the BetterHomesBC PQEAs...</p>
    ...
    <table id="pqeasResults" class="pqeasResults table table--striped">
        ...
        <tbody :class="`page page--${currentPage}`">
            ...
            <!-- Loading Message -->
            <tr v-if="isLoading" class="is-loading no-results">
                <td colspan="100%">
                    <p class="no-results" aria-live='polite'>Loading...</p>
                </td>
            </tr>
    ...
    <div v-if="!isLoading" class='pqeasFilterPagination pqeas-filter__pagination pqeas-filter__pagination--bottom'>
        <template v-if='filteredPqeas.length > 0'>
...
/**
 * Variable to hold the value of the currently selected location.
 *
 * @type {String|null} - The value of the currently selected location or null if not available.
 */
const selectedLoc = selectedLocation.value ? selectedLocation.value : null;
...
/**
 * Fetches pqea data either from localStorage cache (if available and not expired) or from the WordPress API.
 * If data is fetched from the API, it is stored in localStorage for caching purposes.
 *
